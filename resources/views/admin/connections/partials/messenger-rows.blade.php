@php
    /** @var \Illuminate\Support\Collection<int, \App\Models\ChannelAccount> $messenger */
@endphp
@forelse ($messenger as $acc)
    <tr class="align-top border-b border-slate-100 bg-white {{ $acc->is_active ? '' : 'opacity-75' }}">
        ...
        <td class="max-w-[14rem] truncate px-4 py-3 font-mono text-xs text-slate-600" title="{{ $acc->external_id }}">{{ $acc->external_id }}</td>
            ...
            <form method="post" action="{{ route('connections.update', $acc) }}" class="space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <x-input-label :value="__('Display name')" for="name_ms_{{ $acc->id }}" />
                        <x-text-input id="name_ms_{{ $acc->id }}" name="name" type="text" class="mt-1 block w-full" :value="$acc->name" required />
                    </div>
                    <div class="sm:col-span-2">
                        <x-input-label :value="__('Page ID')" for="ext_ms_{{ $acc->id }}" />
                        <x-text-input id="ext_ms_{{ $acc->id }}" name="external_id" type="text" class="mt-1 block w-full font-mono text-sm" :value="$acc->external_id" required />
                    </div>
                    <div class="sm:col-span-2">
                        <x-input-label :value="__('New page access token (leave blank to keep)')" for="tok_ms_{{ $acc->id }}" />
                        <x-text-input id="tok_ms_{{ $acc->id }}" name="access_token" type="password" class="mt-1 block w-full font-mono text-sm" autocomplete="new-password" />
                    </div>
                </div>
...
@empty
    <tr>
        <td colspan="4" class="px-4 py-10 text-center text-sm text-slate-500">{{ __('No Messenger pages yet.') }}</td>
    </tr>
@endforelse
